...
const fetchWishlistStatus = async () => {
  ...
  if (loading) {
    return (
      <section className="relative py-20 min-h-[400px] bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900">
        <div className="flex items-center justify-center min-h-[300px]">
          <div className="text-center">
            ...
            <p className="text-white text-lg">Discovering products...</p>
          </div>
        </div>
      </section>
    );
    ...
    return (
      <section className="relative py-20 bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900">
        <div className="max-w-md mx-auto text-center">
          ...
          <button onClick={fetchFeaturedProducts} className="px-6 py-3 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition">Try Again</button>
        </div>
      </section>
    );
  }
  ...
  return (
    <section ref={sectionRef} className="relative py-20 overflow-hidden bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div ref={headerRef} className="text-center mb-12">
          <h2 className="text-4xl sm:text-5xl font-bold mb-4">
            <span className="bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">Featured Products</span>
          </h2>
          <div className="w-24 h-1 bg-gradient-to-r from-blue-500 to-purple-500 mx-auto mb-4 rounded-full"></div>
          <p className="text-gray-300 text-lg">Handpicked just for you</p>
        </div>
          ...
          {products.map((product) => {
            ...
            return (
              <Link
                ...
                className="group bg-white/10 backdrop-blur-xl rounded-2xl overflow-hidden border border-white/20 hover:border-white/40 hover:transform hover:-translate-y-2 transition-all duration-500"
              >
                <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-gray-800 to-gray-900">
                  {imageUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <span className="text-5xl font-bold text-white/30">{(product.name || 'P').charAt(0)}</span>
                    </div>
                  )}
                  ...
                  {product.stock <= 5 && product.stock > 0 && (
                    <span className="absolute top-3 right-3 px-2 py-1 bg-yellow-500 text-white text-xs rounded-full">Only {product.stock}</span>
                  )}
                  {!inStock && (
                    <span className="absolute top-3 right-3 px-2 py-1 bg-red-500/90 text-white text-xs font-semibold rounded-full">Sold Out</span>
                  )}

                  {/* ✅ Wishlist Heart Button - Top Right (above badges if no stock badge) */}
                    ...
                    <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                      ...
                      >
                        {addingToCart === product._id ? (
                          ...
                          <svg className="w-5 h-5 text-gray-700 hover:text-violet-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div ref={viewAllRef} className="text-center mt-12">
          <Link href="/products" className="inline-flex items-center gap-2 px-8 py-3 bg-white/20 text-white rounded-full hover:bg-white/30 transition-all font-semibold border border-white/30">
            View All Products
