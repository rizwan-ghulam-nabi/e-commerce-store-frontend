...
export default function Hero() {
    ...
    try {
      ...
      if (normalProducts.length > 0) {
        const heroSlides = normalProducts.slice(0, 5).map((product, index) => {
          ...
          // Get the product ID and ensure it's valid
          const productId = product._id || product.id;
          
          // Log the product ID for debugging
          console.log('Product ID for slide:', productId, 'Product name:', product.name);
          ...
          return {
            ...
            bgGradient: colorSchemes[index % colorSchemes.length],
            badge: product.stock <= 5 && product.stock > 0 ? 'Low Stock' : 'New Arrival',
            // Fix the link path - ensure it matches your routing structure
            ctaLink: productId ? `/products/${productId}` : '/products',
          };
        });

        setSlides(heroSlides);
        console.log('Hero slides generated:', heroSlides);
      } else {
        setSlides(FALLBACK_SLIDES);
      }
  ...
  const handleShopNow = (ctaLink) => {
    console.log('Navigating to:', ctaLink);
    if (ctaLink && ctaLink !== '#') {
      router.push(ctaLink);
    } else {
      console.warn('Invalid link:', ctaLink);
  ...
  if (loading) {
    return (
      <section className="relative w-full">
        <div className="w-full h-[300px] xs:h-[350px] sm:h-[400px] md:h-[500px] lg:h-[600px] bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 flex items-center justify-center">
          <div className="text-center px-4">
            ...
            <p className="text-gray-600 dark:text-gray-400 font-medium">Loading amazing deals...</p>
          </div>
        </div>
      </section>
    );
  ...
  <div className="relative z-20 h-full flex items-center">
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
      <div className="text-center max-w-2xl lg:max-w-3xl mx-auto">
        ...
        {slide.highlight && (
          <p className="text-xl sm:text-2xl md:text-3xl text-violet-300 font-semibold mb-4">
            {slide.highlight}
          </p>
        )}
        
        {/* Description - centered */}
        <p className="text-gray-200 text-sm sm:text-base md:text-lg mb-6 max-w-md mx-auto">
          {slide.description}
        </p>
        ...
        {slide.price && (
          <div className="flex items-center justify-center gap-3 mb-6">
            ...
            {slide.discount && (
              <span className="bg-green-500 text-white px-2.5 py-1 rounded-full text-sm font-bold">
                {slide.discount}
              </span>
            )}
          </div>
        ...
        <div className="flex justify-center">
          <button 
            ...
            className="group inline-flex items-center gap-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white px-6 sm:px-8 py-3 rounded-full font-semibold transition-all duration-200 shadow-lg hover:shadow-violet-500/25 cursor-pointer"
          >
            <span>{slide.ctaText}</span>
    ...
    >
      <div className="relative w-full h-[400px] sm:h-[500px] md:h-[600px] overflow-hidden">
        {slides.map((slide, idx) => (
          <div
            ...
            style={{
              transform: idx < currentIndex ? 'translateX(-100%)' : undefined,
      ...
      {slides.length > 1 && (
        <>
          ...
          <button 
            onClick={prevSlide} 
            className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-30 bg-black/50 hover:bg-black/70 text-white rounded-full p-2 sm:p-3 transition-all backdrop-blur-sm"
            aria-label="Previous"
          >
          ...
          <button 
            onClick={nextSlide} 
            className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-30 bg-black/50 hover:bg-black/70 text-white rounded-full p-2 sm:p-3 transition-all backdrop-blur-sm"
            aria-label="Next"
          >
            <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
          ...
          {/* Counter */}
          <div className="absolute bottom-4 right-4 z-20 bg-black/50 backdrop-blur-sm px-2 py-1 rounded-full">
